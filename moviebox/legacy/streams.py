...
from moviebox.legacy.extractors.json_models import ItemJsonDetailsModel, PostListItemSubjectModel
...
__all__ = ['MediaFileDownloader', 'CaptionFileDownloader', 'DownloadableMovieFilesDetail', 'DownloadableTVSeriesFilesDetail', 'resolve_media_file_to_be_downloaded']
...
class BaseDownloadableFilesDetail(BaseContentProviderAndHelper):
    ...
    def __init__(self, session: Session, item: SearchResultsItem | ItemJsonDetailsModel):
        ...
        self._item: SearchResultsItem | PostListItemSubjectModel = item.resData.postList.items[0].subject if isinstance(item, ItemJsonDetailsModel) else item
    ...
    async def get_content(self, season: int, episode: int) -> dict:
        ...
        content = await self.session.get_with_cookies_from_api(url=self._url, params=self._create_request_params(season, episode), headers=request_header)
        return content
