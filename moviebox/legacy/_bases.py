...
class BaseContentProvider(ABC):
    """Provides easy retrieval of resource from provider"""
...
class BaseFileDownloaderAndHelper(FileDownloaderHelper, BaseFileDownloader):
    ...
    @classmethod
    def create_final_dir(cls, working_dir: Path, search_results_item: object, season: int, episode: int, test: bool, group: bool):
        if group and season and episode:
            working_dir = Path(working_dir)
            assert working_dir.exists(), f'The chosen working directory does not exist - {working_dir}'
            final_dir = working_dir.joinpath(f'{search_results_item.title} ({search_results_item.releaseDate.year})', f'S{season}')
            if not test:
